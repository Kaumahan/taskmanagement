...
<template>
    ...
    <form @submit.prevent="addProject" class="flex gap-2">
        <input 
            ...
            type="text" 
            placeholder="e.g., Marketing Campaign, Home Renovation" 
    ...
    <div v-if="projectForm.errors.name" class="text-red-500 text-xs mt-1 italic">
        ...
        <div class="mb-8">
            <div v-if="!isEditingProject" class="flex items-center gap-2">
                ...
                <button @click="isEditingProject = true" class="text-blue-600 px-2 py-1 text-sm border border-blue-600 rounded">Edit</button>
                <button @click="deleteProject" class="text-red-600 px-2 py-1 text-sm border border-red-600 rounded">Delete</button>
            </div>
        ...
        >
            <template #item="{ element }">
                <div class="flex items-center p-4 bg-white shadow rounded border border-gray-100">
                    ...
                    <button @click="router.delete(route('tasks.destroy', element.id))" class="text-red-400">Delete</button>
                </div>
            </template>
        </draggable>
    </div>
